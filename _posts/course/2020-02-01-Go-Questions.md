...
1. [Slice] 如何给字符串 slice 排序？sort.Strings(s)
...
1. [Map] Map 对象怎么声明和初始化？`ages := make(map[string]int)` == `map[string]int{}`，也可以直接通过 map 字面量进行初始化 `ages = map[string]int{ "alice":31, "charlie":14}`
...
1. [Map] 如何遍历一个 map ？`for name := range(ages)`；`for name, age := range(ages)`，迭代顺序不固定。

1. [Map] map 的零值 nil 有什么注意事项？不可以为零值中设置元素，必须先初始化。`var ages map[string]int; ages = map[string]int{}; ages["hello"] = 1`

1. [Map] 当一个键不存在的时候，map 取索引会返回零值，如何能知道键值是否存在？`if ages,ok:=ages["bob"];!ok { not exist... }`

1. [Map] 怎么实现集合？只用 map 的键。但没有集合运算库函数。

1. [Map] 当希望键是 slice 时，怎么办？添加 hash 核函数 k，对 s1 == s2 时，k(s1) 才会 == k(s2)
