...
### Memory Leak Case: 953422410350611
- Symptom
	- XP
...
- Analysis
	...
	- 进一步分析
		...
		- Address -f Unknown占了1.76G(1895484928)，共3195个Block，其中大小为0x240000和0x900000的Block有230个，共1.07G，占了主要部分
		- 尝试分析0x240000和900000这两个类型的block，发现这些空间内存储的数据看起来像图片（这些凌乱而又部分连续的数据，如果不是图片，我想不出来还有什么可能）。首先肯定不是ASCII或者Unicode，一定是Binary，但如果是一般的Binary又不应该这么有规律（部分连续）。这样看来，可能是部分和显示图片有关的内存用完没有释放（`!address -F:Unk`，`!heap -stat`，`!mex.strings`）
				...
				3143eb78 !!!!!!
				3143ec50 """"""""""""""""""""""""""""""######$$$%%%&&&&&&""""""######$$$%%%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&))))))((('''%%%$$$############$$$%%%'''((())))))&&&'''(((***+++---...///,,,...000444888;;;>>>???;;;<<<===@@@BBBEEEFFFGGGHHHJJJLLLOOORRRUUUXXXYYYZZZ___gggmmmpppnnniiifffjjjjjjkkkkkklllmmmnnnnnn{{{xxxwwwyyy~~~
				3143ed85 |||~~~
				3143f34c }}}||||||}}}
				3143f517 ~~~}}}|||
				3143f53e ~~~|||||||||~~~~~~||||||}}}}}}~~~~~~~~~~~~{{{||||||}}}~~~
		...
		- 这里不存在GDI Objects泄漏（几个Dump中并没有增加趋势），需要`!load gdikdx`，[参考](http://www.osronline.com/article.cfm?id=173)
				...
				Warning: Event callbacks have not been registered.
				   If dumpobj is the first extension used, use .load or !load in the future.
				   Caching is disabled for this use of !dumpobj.
				Extension 'dumpobj' is not fully converted.
				object list for ALL type objects owned by PID 0x68c
				   I,   handle,   Lock, sCount,  pid,       pv,     objt, unique,  Flags,    pUser,    Tlock
                  
				  9c, 2a10009c,      0, Unread,  68c, e1334308,    BRUSH,   2a10,      0, 00910f90, 00910f90
				  fc, 4d0100fc,      0, Unread,  68c, e7f5e9c8,       DC,   4d01,      0, 00900000, 00900000
				  ff, 970a00ff,      0, Unread,  68c, e60199f0,    LFONT,   970a,      0, 0c0f1438, 0c0f1438
				211, ff0a0211,      0, Unread,  68c, e4033008,    LFONT,   ff0a,      0, 0c0f1458, 0c0f1458
				......
